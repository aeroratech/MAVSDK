...
public:
    ...
    grpc::Status SetZoomRange(
        grpc::ServerContext* /* context */,
        const rpc::camera::SetZoomRangeRequest* request,
        rpc::camera::SetZoomRangeResponse* response) override
    {
        if (_lazy_plugin.maybe_plugin() == nullptr) {
            if (response != nullptr) {
                auto result = mavsdk::Camera::Result::NoSystem;
        ...
        if (request == nullptr) {
            LogWarn() << "SetZoomRange sent with a null request! Ignoring...";
            return grpc::Status::OK;
        }

        auto result = _lazy_plugin.maybe_plugin()->set_zoom_range(request->range());
